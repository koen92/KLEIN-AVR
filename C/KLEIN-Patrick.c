...
{
    unsigned char a[4]; // a = r
    unsigned char b[4]; // b becomes 2 * a
    ...
    {
        a[c] = r[c];
        h = (unsigned char)((signed char)r[c] >> 7); /* arithmetic right shift, thus shifting in either zeros or ones */                    // filled with most significant bits
        b[c] = r[c] << 1; /* implicitly removes high bit because b[c] is an 8-bit char, so we xor by 0x1b and not 0x11b in the next line */ // thus *2
        b[c] ^= 0x1B & h; /* Rijndael's Galois field */     // if h = 0000000 then you do nothing, else if h = 1111111 you do a xor with 0x1B
        //b[c] = galois_x4_1( ((uint8_t) b[c]) );
    }
